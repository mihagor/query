...
--        count(pca.attribute_name) over (partition by pc.level04_name),
...
from    f_sales_all_daily f
        inner join d_company co                         on f.company_sid = co.company_sid               and co.company_code = '812968'
        inner join v_profit_center_bi pc                on f.sales_point_sid = pc.profit_center_sid     and pc.hierarchy_ext_refr = 'PC|PIS' 
        inner join v_product_bi p                       on f.product_sid = p.product_sid                and p.hierarchy_ext_refr = 'PR|SAP' --'PR|SAP'
...
where   accounting_date_sid between 20200601 and 20200608
and     f.is_own_usage = 0
and     p.level01_code <> ('06')
--and     p.level01_code in ('05')
--and     p.level02_code = '3408|GR'
--and     p.level02_code = '0102'
and     pc.level03_code in ('3203') --, '3202' 
--and     pc.level04_code = '3229' 
--and     p.level02_code = '0533'
--and     (pca.attribute_name in ('BS GEOGRAFSKO', 'BS po regijah') or pca.attribute_name is null) --, 'LOKACIJA BS'
